...
import { sdk } from '../sdk'
import { getWebUiInterfaceUrls, normalizeOrigin } from '../utils'
...
export const setupHubConfig = sdk.setupOnInit(async (effects) => {
  ...
  const primaryUrl = normalizeOrigin(hubConfig?.primaryUrl ?? '')

  const primaryUrlAvailable =
    primaryUrl !== null && availableUrls.includes(primaryUrl)

  if (!primaryUrlAvailable) {
    await sdk.action.createOwnTask(effects, setHubConfig, 'important', {
      reason: i18n(
